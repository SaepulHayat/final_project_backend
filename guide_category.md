...
- **Serialization:** Add a `to_dict()` method to control API output:
  ...
  def to_dict(self):
      return {
          ...
          # Add 'book_count': len(self.books) if needed, but be mindful of performance
      }

  # Consider a simpler version for book lists
      ...
      def get_books_by_category(self, category_id, args):
          ...
          try:
              ...
              return success_response(
                  ...
                  data={
                      "books": [book.to_simple_dict() for book in paginated_books.items], # Use a simple book representation
                      "total": paginated_books.total,
                      "pages": paginated_books.pages,
                      "current_page": paginated_books.page,
                      "category": category.to_simple_dict() # Include category info
                  },
                  status_code=200
              )
          except Exception as e:
...
- **Responsibilities:** Encapsulates all business logic related to categories, interacts with the database via the `Category` model, performs validation (including uniqueness checks), handles relationships (implicitly for deletion in M2M), and manages specific errors.
...
- **Responsibilities:** Defines API endpoints, handles HTTP request/response cycle, parses request data (JSON body, query args), calls the `CategoryService`, applies security decorators (`@jwt_required`, `@roles_required`), and uses `create_response` to format the final JSON output with appropriate HTTP status codes.
...
- **Uniqueness Constraint:**
  ...
  - The `create_category` and `update_category` service methods include explicit, case-insensitive checks _before_ attempting the database operation to provide a clearer error message (409 Conflict). The check compares against the _title-cased_ version of the input name.
...
- **Serialization (`to_dict()`):**
  - Implement `to_dict()` and potentially `to_simple_dict()` methods on the `Category` model to control API response structure. Ensure related models (`Book`) also have appropriate serialization methods (`to_simple_dict()` used in the example).
